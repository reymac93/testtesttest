...
GLOBAL $msg_log;
		...
		$j = 0;
		$result = array();
                while ($record = mysqli_fetch_array( $sql_result )) {
                	$result[ $j++ ] = $record;
		} // while
		if ($j == 0) {
			unset( $result );
...
//	$xml_fields[ $j ][ EXPR     ] = "(\$camp_array['Customer Name'] == '" . TELAMERICA . "')";
	...
	//$xml_fields[ $j ][ EXPR     ] = "(string)\date_format($campaign->dateRange->endDate,'Y-m-d')";
...
// version fields need to be first (to use $j+1)
	...
	//CADENT = Multiply 0 = CINDEX 606
	//NCC = Multiply 10 = CINDEX 6
	
	$xml_fields[ $j ][ FLD_NAME ] = "Discount";
	$xml_fields[ $j ][ EXPR     ] = "\$camp_array['Customer Record']['Discount'] * 10";
	///Edited By Reynan G. Macasaquit //Boss Instruction
	//$xml_fields[ $j ][ EXPR     ] = "\$camp_array['Customer Record']['Discount'] * 10";
	$j++;
...
date_format( date_modify( date_create( \$one_line['StartDate'], new DateTimeZone( "GMT" ) ), '+'.\$addenddate.' days' ), 'Y-m-d' )
__EOF__;
	$j++;
...
//	$xml_fields[ $j ][ EXPR     ] = "substr( (string)\$spot['id'], -4 )";
	...
	$xml_fields[ $j ][ FLD_NAME ] = "UseStartDate";
	$xml_fields[ $j ][ EXPR     ] = 1;  ///reynan update
	$j++;
...
//if ($DEBUG) echo "loading XML\n";
	...
	if ($success) {
		...
		if (!$success)
			echo "XML file is the wrong documentTypes.\n";
	}
