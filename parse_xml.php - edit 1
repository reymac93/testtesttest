...
GLOBAL $msg_log;
	...
	$qry = "SELECT * FROM site_operators WHERE Operator =  '$oper_name' " . "AND SysCode = '$sys_code'";
...
GLOBAL $msg_log;
    ...
    if (($sql_result = mysql_query( $qry, $db_conn )) && ($record = mysql_fetch_array( $sql_result ))) 
	{
        $result = $record;
...
function parse_campaign( $campaign, &$camp_array )
...
//	$xml_fields[ $j ][ EXPR     ] = "(\$camp_array['Customer Name'] == '" . TELAMERICA . "')";
	...
	$xml_fields[ $j ][ EXPR     ] = "(\$camp_array['TELAMERICA']) ? ((string)('' . \$campaign->product->name)) : ((string)('' . \$campaign->product->name))";
	$j++;
...
function parse_contract_header( $campaign, $system_order, $camp_array, &$hdr_array )
...
// version fields need to be first (to use $j+1)
	...
	$xml_fields[ $j ][ EXPR     ] = "salesman_record( '" . SALESMAN_NAME . "', '" . OPERATOR_NAME . "' )";
	$j++;
...
function parse_contract_detail( $campaign, $system_order, &$det_array )
...
date_format( date_modify( date_create( \$one_line['StartDate'], new DateTimeZone( "GMT" ) ), '+6 ko days' ), 'Y-m-d' )
__EOF__;
	$j++;
...
// other run_day cases would go here
			...
			default:
				$success = FALSE;
				message_log_append( $msg_log, "Invalid run day format: '$run_day'",	MSG_LOG_ERROR );
			} // switch
			$dow++;
		}
...
// day codes stipulated by the XML spec:
		...
		{
			$success = FALSE;
			message_log_append( $msg_log, "Invalid start day in detailLineID " . $detail_line['detailLineID'] . ": '$start_day_code'", MSG_LOG_ERROR );
//		} else {
...
//}
			if (!$success) 
			{
				message_log_append( $msg_log, "Can't parse campaign from XML file",	MSG_LOG_ERROR );
			} // if
		} 
		else 
		{
			message_log_append( $msg_log, "Can't locate 'campaign' node in XML file", MSG_LOG_ERROR );
		}
	} // if
//echo "4: " . ($success ? "parse_xml TRUE\n" : "parse_xml FALSE\n" );
	if ($success) 
	{
		...
		$n_sys_ord = 0;
		while ($success && isset( $sys_ords[ $n_sys_ord ] ) && !is_null( $sys_ords[ $n_sys_ord ] )) 
		{
...
//echo "4z: too many contracts\n";
				$success = FALSE;
				message_log_append( $msg_log, "XML file contains too many contracts", MSG_LOG_ERROR );
			} // if
		} // while
