...
//					$key . $key . '</label>' . "\n";
		...
		$comDate= strtotime('2010-12-01');?></br><?php
		$startDate = strtotime($sdate);?></br><?php
		$endDate = strtotime($edate);?></br><?php
		
		///$p_import = $reynan;  //reynan need to enable this
		$p_import = $p_import && ($camp_spots > 0);
		//if ($p_import && ($camp_loglevel < MSG_LOG_ERROR)) 
		
		if  (($p_import && ($camp_loglevel < MSG_LOG_ERROR)) && $startDate > $comDate && $endDate > $comDate)
		{
